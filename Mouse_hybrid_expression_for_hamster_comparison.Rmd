...
knitr::opts_knit$set(root.dir = "~/Library/CloudStorage/Dropbox-Personal/1_Denver/Research/Hamsters/Sterile_hamster_hybrids/github_repository_sterile_hamster_hybrids_post-revisions/")
unlink(".RData")
all_labels()
options(scipen = 999)
...
all_exp <- rownames(dgeMouse$genes)
#expressed across parents
MusP_DomP_all_exp <- rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("P-", colnames(rpkm.nonLog))] > rpkm.cutoff)  >= min.reps, ]) 

#expressed within M. m. musculus
MusP_all_exp <- rownames(dgeMusP$genes)
#expressed within M. m. domesticus
DomP_all_exp <- rownames(dgeDomP$genes)
#expressed within mus x dom hybrids
Hyb_all_exp <- rownames(dgeMSHyb$genes)

#expressed across all crosses for each stage
Hyb_Comb_SP_exp <- rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("SP", colnames(rpkm.nonLog))] > rpkm.cutoff)  >= min.reps, ]) # 15258
Hyb_Comb_LZ_exp <- rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("LZ", colnames(rpkm.nonLog))] > rpkm.cutoff)  >= min.reps, ]) # 14500
Hyb_Comb_DIP_exp <- rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("DIP", colnames(rpkm.nonLog))] > rpkm.cutoff)  >= min.reps, ]) # 13647
Hyb_Comb_RS_exp <- rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("RS", colnames(rpkm.nonLog))] > rpkm.cutoff)  >= min.reps, ]) # 15453
