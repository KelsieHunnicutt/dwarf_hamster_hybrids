...
knitr::opts_knit$set(root.dir = "~/Library/CloudStorage/Dropbox/1_Denver/Research/Hamsters/Sterile_hamster_hybrids/github_repository_sterile_hamster_hybrids_post-revisions/")
all_labels()
unlink(".RData")
options(scipen = 999)
...
#expressed across all crosses for each stage
Hyb_Comb_SP_exp <- annotations[annotations$gene %in% 
                                 rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("SP", colnames(rpkm.nonLog))] 
                                                              > rpkm.cutoff) >= min.reps, ]),c("gene", "mouse_ensemble_ID")] # 18905
Hyb_Comb_LZ_exp <- annotations[annotations$gene %in% 
                                 rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("LZ", colnames(rpkm.nonLog))] 
                                                              > rpkm.cutoff) >= min.reps, ]),c("gene", "mouse_ensemble_ID")] # 18433
Hyb_Comb_DIP_exp <- annotations[annotations$gene %in% 
                                  rownames(rpkm.nonLog[rowSums(rpkm.nonLog[,grep("DIP", colnames(rpkm.nonLog))] 
                                                               > rpkm.cutoff) >= min.reps, ]),c("gene", "mouse_ensemble_ID")] # 16403
...
write.csv(Hyb_Comb_SP_exp,  "exported_files/Hamster_SP_exp_genes.csv")
write.csv(Hyb_Comb_LZ_exp,  "exported_files/Hamster_LZ_exp_genes.csv")
write.csv(Hyb_Comb_DIP_exp, "exported_files/Hamster_DIP_exp_genes.csv")
...
test_for_overlap_enrichment <- function(stage) {
  ...
  Mouse_exp <- read.csv(paste("exported_files/Mouse_", stage ,"_exp_genes.csv", sep = ""))$gene
  Hamster_exp <- get(paste("Hyb_Comb_", stage, "_exp", sep = ""))[,2]
  
  #a list of genes that are expressed in the given stage in either system
  exp_in_either <- unique(c(Mouse_exp, Hamster_exp))
  
  #a list of the trangressively expressed mouse genes
  ...
  under.p <- phyper(length(Both.trans), 
                    length(Hamster.trans$mouse_ensemble_ID), 
                    length(setdiff(exp_in_either,Hamster.trans$mouse_ensemble_ID)), 
  ...
  #calculate number of observed and expected genes
  exp <- round((length(Hamster.trans$mouse_ensemble_ID)/
                  length(setdiff(exp_in_either,Hamster.trans$mouse_ensemble_ID)))*dim(Mouse.SP.trans)[1],
               digits = 0)
  ...
  return(c(stage, under, over, exp, obs, 
           length(Mouse.trans$gene), length(Hamster.trans$mouse_ensemble_ID),
...
#combine dataframes
overlap_definitions_df <- data.frame(do.call(rbind, list(overlap_definitions_SP_df, overlap_definitions_LZ_df, overlap_definitions_DIP_df)))
